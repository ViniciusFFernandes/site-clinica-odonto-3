// app.js — interações do site (header no scroll, menu, reveal, scrollspy, especialidades)
(function () {
  "use strict";
  ...
  var tabs = document.querySelectorAll("[data-esp-tab]");
  var panels = document.querySelectorAll("[data-esp-panel]");
  function activate(key) {
    tabs.forEach(function (t) {
      var on = t.getAttribute("data-esp-tab") === key;
      t.setAttribute("aria-selected", on ? "true" : "false");
      t.classList.toggle("esp-tab-active", on);
    });
    panels.forEach(function (p) {
      var on = p.getAttribute("data-esp-panel") === key;
      p.classList.toggle("hidden", !on);
    });
  }
  tabs.forEach(function (t) {
    t.addEventListener("click", function () { activate(t.getAttribute("data-esp-tab")); });
  });
